...
def predict_co2_per_capita(data, year, k):
    ...
    for country in prediction_filtered['country'].unique(): #predict the co2 emission country by country
        ...
        emission_per_capita = country_data_reverse['emission_per_capita'] - country_data_reverse['emission_per_capita'].shift(periods=-1) #find the difference between "this year" emission per capita and "last year" emission per capita
...
def uncertainty_co2_per_capita(data, year, k):
    uncertainty = for_question_4(data)

    uncertainty_statistics = {}

    for country in uncertainty['country'].unique():
        country_data = uncertainty[uncertainty['country'] == country]
        country_data = country_data[country_data['year'] < year].sort_values('year')[-k:]

        country_data['median'] = country_data['emission_per_capita'].median()
        country_data['10thpercentile'] = country_data['emission_per_capita'].quantile(q=0.1)
        country_data['90thpercentile'] = country_data['emission_per_capita'].quantile(q=0.9)

        country_data = country_data[['country', '10thpercentile', 'median', '90thpercentile']]

        median_set = set(country_data['median'].fillna(0).values)
        _10thpercentile_set = set(country_data['10thpercentile'].fillna(0).values)
        _90thpercentile_set = set(country_data['90thpercentile'].fillna(0).values)

        [median] = median_set
        [_10thpercentile] = _10thpercentile_set
        [_90thpercentile] = _90thpercentile_set
        
        uncertainty_statistics[country] = median, _10thpercentile, _90thpercentile
